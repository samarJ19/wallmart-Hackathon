...
router.post("/create", requireAuth(), async (req, res) => {
  try {
    ...
    if (name.length > 50) {
      return res
        .status(400)
        .json({ error: "Group name must be 50 characters or less" });
    }

    if (maxMembers < 2 || maxMembers > 50) {
      return res
        .status(400)
        .json({ error: "Max members must be between 2 and 50" });
    }
...
// POST /api/groups/invite - Send invitation to join group
router.post("/invite", requireAuth(), async (req, res) => {
  try {
    ...
    if (!groupChatId || !userEmail) {
      return res
        .status(400)
        .json({ error: "Group chat ID and user email are required" });
    }
    ...
    if (!groupChat.isActive) {
      return res
        .status(400)
        .json({ error: "Cannot invite users to inactive group" });
    }
    ...
    if (groupChat._count.members >= groupChat.maxMembers) {
      return res
        .status(400)
        .json({ error: "Group has reached maximum member limit" });
    }
...
// POST /api/groups/upload-image - Upload image to chat
router.post(
  "/upload-image",
  requireAuth(),
  upload.single("image"),
  async (req, res) => {
    try {
      ...
      if (!membership) {
        return res
          .status(403)
          .json({ error: "Not authorized to send messages to this group" });
      }
...
router.get("/my-groups", requireAuth(), async (req, res) => {
  try {
    ...
    const userGroups = await prisma.groupChat.findMany({
      ...
      orderBy: { updatedAt: "desc" }, // More logical for group chats
    });

    const userGroupsWithMemberInfo = userGroups.map((group) => {
    ...
    res.status(200).json({
      ...
      groups: userGroupsWithMemberInfo, // or just userGroups if you don't need member info
      userId,
    });
  } catch (error) {
...
// GET /api/groups/admin/my-groups - Get user's group detail where he is admin
router.get("/admin/my-groups", requireAuth(), async (req, res) => {
  try {
    const user = await getUserFromAuth(req);
    const { prisma } = req;
    if (!user) {
    ...
    const adminGroupChats = await prisma.groupChat.findMany({
      where: {
        members: {
          some: {
            userId: userId,
...
// GET /api/groups/:id - Get specific group chat details
...
router.get("/:id/messages", requireAuth(), async (req, res) => {
  try {
    ...
    if (!membership) {
      return res
        .status(403)
        .json({ error: "Not authorized to view messages in this group" });
    }
    ...
    res.status(200).json({
      ...
      messages: messages, // Reverse to show oldest first, why oldest first, when we rendering messages we should show new messages first
      pagination: {
        page: parseInt(page),
...
// DELETE /api/groups/:id/leave - Leave a group chat
        ...
        await prisma.chatMessage.create({
          data: {
            ...
            content: `${user.name} left the group. ${
              otherMembers[0].user?.name || "Member"
            } is now the admin.`,
            messageType: "system",
            metadata: {
...
// DELETE /api/groups/:groupId/remove/:userId - Remove a user from group (admin only)
router.delete("/:groupId/remove/:userId", requireAuth(), async (req, res) => {
  try {
    ...
    if (!adminMembership) {
      return res
        .status(403)
        .json({ error: "Only group admins can remove users" });
    }
