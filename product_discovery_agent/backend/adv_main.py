...
app = FastAPI(title="Advanced Product Discovery Chatbot API", version="2.0")
...
async def lifespan(app: FastAPI):
    """Lifespan context to manage startup and shutdown events"""
    global conversational_agent
    await startup_event()  # Your custom initialization logic
...
async def startup_event():
    ...
    global advanced_retriever, query_processor
...
class EnhancedConversationalAgent:
    ...
    async def process_message(self, message: str, session_memory: ConversationMemory) -> ChatResponse:
        ...
        try:
            # Process the query
            processed_query = self.query_processor.process_query(message, session_memory.get_context())
            
            # Update conversation memory
            session_memory.add_interaction(message, "", processed_query)
            ...
            session_memory.interactions[-1]['assistant'] = response_text
    ...
    def calculate_confidence(self, processed_query: Dict[str, Any], search_results: List[SearchResult]) -> float:
        ...
        entities = processed_query['entities']
        if entities:
            exact_matches = sum(1 for result in search_results if result.score > 0.8)
...
async def get_recommendations(request: ProductRecommendationRequest):
    ...
    try:
        ...
        # Use retriever to find similar products
...
async def search_products(query: dict):
    ...
    try:
        # Create a mock session for search
        ...
        processed_query = {
            ...
            'intent': 'search',
            'entities': filters,
            'price_sensitivity': query.get('price_sensitivity', 'neutral'),
            'context': {}
        }
        
        # Perform search
        search_results = await advanced_retriever.search(processed_query)
...
async def reload_products_endpoint():
    ...
    try:
        ...
        # Reinitialize the retriever with new products
