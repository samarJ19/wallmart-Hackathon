...
/*
import { connectWebSocket, getSocket, trackProductView, isConnected } from './websocket';
...
const MyComponent = () => {
  ...
  const handleProductClick = (productId: string) => {
    ...
    // socket?.emit('user-interaction', { userId, productId, action: 'view' });
  };

  return (
    <div>
      <p>Socket Status: {isConnected() ? '🟢 Connected' : '🔴 Disconnected'}</p>
      <button onClick={() => handleProductClick('product123')}>
        View Product
      </button>
    </div>
  );
